    ...
    for backend, name in backends:
        try:
            ...
            if cam.isOpened():
                ...
                camera = cam
                print(f"📷 Camera initialized via {name}")
                return
        except:
            pass
...
def log_pet(label, conf):
    write_log(f"PHÁT HIỆN PET: {label} ({conf:.2f})")
...
def arduino_simulation_loop():
    ...
    while True:
        ...
        if action == "pir":
            ...
            if last_pir == 1:
                log_pir()
                behavior_score = min(100, behavior_score + 2)

        elif action == "rfid":
            last_rfid = random.choice(["PET001","PET002","DOG789","CAT123"])
            log_rfid(last_rfid)
