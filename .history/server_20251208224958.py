...
PET_MODEL = YOLO("yolov8n.pt")   # đổi thành yolov8s.pt nếu cần chính xác hơn
PET_THRESHOLD = 0.25
...
def log_motion():
    ...
    print("📌 LOG MOTION:", msg)


def log_pet(label, conf):
    """Ghi log khi phát hiện thú cưng."""
    msg = f"{datetime.now().strftime('%H:%M:%S')} - Phát hiện thú cưng: {label} ({conf:.2f})"
    with open(LOG_FILE, "a", encoding="utf-8") as f:
        f.write(msg + "\n")
    print("🐾 LOG PET:", msg)
...
def camera_loop():
    ...
    global camera   # FIX lỗi scope

    error_count = 0

    while True:
        try:
            # Nếu camera mất tung tích → reconnect
            ...
            # ==================================================
            # YOLO PET DETECTION (dog/cat only)
            # ==================================================
            pet_detected_flag = False
