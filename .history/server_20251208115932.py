...
def motion_stats():
    ...
    with open(LOG_FILE, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # CHUẨN HOÁ: chỉ lấy HH
            try:
                time_part = raw_time.strip().split(" ")[-1]   # lấy HH:MM:SS
                hour = time_part.split(":")[0].strip()         # lấy HH
            except:
                continue

            stats[hour] = stats.get(hour, 0) + 1

    result = [{"hour": h, "count": stats[h]} for h in sorted(stats.keys(), key=int)]
