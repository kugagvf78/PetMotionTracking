...
def motion_stats():
    ...
    # Đọc log và đếm số lần theo giờ
    with open(LOG_FILE, "r", encoding="utf-8") as f:
        for line in f:
            if " - " not in line:
                continue
            time_part = line.split(" - ")[0]  # Lấy HH:MM:SS
            hour = time_part.split(":")[0]    # Lấy HH
            stats[hour] = stats.get(hour, 0) + 1

    # Trả về sorted theo giờ
    result = [{"hour": h, "count": stats[h]} for h in sorted(stats.keys())]
