...
# LOAD YOLO PET MODEL
# ================================
print("🔍 Loading YOLO pet model...")
PET_MODEL = YOLO("runs/detect/train/weights/best.pt")   # sửa nếu sai đường dẫn
PET_THRESHOLD = 0.55
    ...
    while True:
        try:
            ...
            if not ret or frame is None:
                error_count += 1
                print(f"⚠️ Lỗi đọc frame ({error_count}/{max_error})")
            ...
            results = PET_MODEL(frame, stream=True, verbose=False)
            for r in results:
                for box in r.boxes:
            ...
            # Hiển thị overlay thông tin thú cưng
            cv2.putText(frame, f"Pet: {pet_label} ({pet_conf:.2f})", (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 200, 0), 2)

            # ================================
            # Cập nhật frame để stream
            # ================================
            with camera_lock:
