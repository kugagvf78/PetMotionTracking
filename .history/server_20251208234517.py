...
def log_rfid(tag):
    write_log(f"RFID: thú cưng mang thẻ {tag}")
        ...
        if last_gray is not None:
            ...
            for c in contours:
                if cv2.contourArea(c) > 800:
                    cv2.rectangle(frame, cv2.boundingRect(c)[0:2]+tuple(map(sum,zip(cv2.boundingRect(c)[0:2],cv2.boundingRect(c)[2:4]))),
                                  (0,255,0), 2)
...
def arduino_simulation_loop():
    ...
    while True:
        ...
        # RFID simulation
            ...
            last_rfid = random.choice(["PET001","PET002","DOG789"])
            log_rfid(last_rfid)
...
if __name__ == "__main__":
    print("🚀 STARTING PET SYSTEM — MODE D (PIR + YOLO + RFID)")
    init_camera()
