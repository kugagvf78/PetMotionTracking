...
def init_camera():
    ...
    for backend, name in backends:
        ...
        try:
            ...
            print(f"✅ Camera hoạt động với backend {name}")
            camera = cam
            return True
    ...
    print("❌ TẤT CẢ backend đều thất bại!")
    return False
...
# ===============================
def camera_loop():
    global latest_frame, pir_triggered, last_rfid, last_pet_detect, behavior_score
    ...
    while not should_stop:
        ...
        if last_gray is not None:
            ...
            for c in contours:
                if cv2.contourArea(c) > 900:  # tăng độ nhạy
                    log_motion()
                    break
